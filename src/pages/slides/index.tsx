...
export default function BungeobbangSlides() {
   ...
   */
  const { encounter, infringedParts, encountered } =
    useContext(InfringementContext);
  /**
   * ! Use with caution ! Use with caution ! Use with caution ! Use with caution ! Use with caution ! Use with caution ! Use with caution ! Use with caution ! Use with caution
   */

  const isInfringed = !!infringedParts;
  ...
  return (
    <button
      style={{
        ...
        border: 'none',
        outline: 'none',
        background: encountered ? 'black' : 'white',
        width: '100vw',
        height: '100vh',
        display: 'flex',
        alignItems: 'center',
    ...
    >
      {isInfringed && isLastSlide && encountered ? (
        <div className={styles.parts}>
          <img className={styles.part} src={infringedParts[0]} />
          <img className={styles.part} src={infringedParts[1]} />
          <img className={styles.part} src={infringedParts[2]} />
          <img className={styles.part} src={infringedParts[3]} />
        </div>
      ) : (
        slides[index]
      )}
